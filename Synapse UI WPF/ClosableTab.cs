...
class ClosableTab : TabItem
{
	...
	public string Title
	{
		...
		get
		{
			return ((CloseableHeader)Header).label_TabTitle.Content as string;
		}
	}
	...
	{
		TabControl control = ((TabControl)Parent);
		if (control.SelectedItem != this)
		{
			File.Delete($"{Constants.currentDir}/bin/tabs/{Title}.txt");
			control.Items.Remove(this);
		} else
		{
			MessageBox.Show("Cant Close Currently Selected Tab!");
		}
	}
